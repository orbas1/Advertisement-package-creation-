          ...
          child: Column(
            ...
            children: [
              ...
              Slider(
                ...
                onChangeEnd: (value) => context.read<ForecastCubit>().fetch({'budget': value, 'duration': duration}),
              ),
              ...
              Slider(
                ...
                onChangeEnd: (value) => context.read<ForecastCubit>().fetch({'budget': budget, 'duration': value}),
              ),
              ...
              BlocBuilder<ForecastCubit, ForecastState>(
                builder: (context, state) {
                  ...
                  final metrics = state.forecast?.metrics;
                  return Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Wrap(
                          ...
                          runSpacing: 12,
                          children: [
                            _MetricCard(label: 'Impressions', value: metrics?.impressions.toString() ?? '--'),
                            _MetricCard(label: 'Clicks', value: metrics?.clicks.toString() ?? '--'),
                            _MetricCard(label: 'Conversions', value: metrics?.conversions.toString() ?? '--'),
                            _MetricCard(label: 'Cost', value: metrics?.cost.toString() ?? '--'),
                          ],
                        ),
                        const Spacer(),
                        ElevatedButton(
